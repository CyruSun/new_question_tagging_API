...
def home(advice=advice):
    ...
    return render_template('home.html', home_title=home_title,
                           abstract=abstract, advice=advice)
# %%
@app.route('/form')
def form(advice=advice):
...
def fill_df_matching_keywords(df_tofill, st_match_words=st_match_words,
    ...
    df_tofill.loc[:, 'Joint_Question_Words'] = df_tofill.loc[
            :, 'Title'] + " " + df_tofill.loc[:, 'Body'].map(str)
    # Delete the html markups
    df_tofill.loc[:, 'Joint_Question_Words'] = df_tofill[
        'Joint_Question_Words'].apply(lambda body_str: clean.strip_tags(
            body_str))
    # The all keywords
    ...
    df_tofill.loc[:, 'Question_Words'] = df_tofill[
            'Question_Words'].apply(lambda x: {elmnt for elmnt in
                                    set(x) if elmnt not in unwanted})
    # Convert to set
    df_tofill['Question_Words'] = df_tofill.apply(
        lambda row: set(row['Question_Words']), axis=1)
    # Find the matching keywords
    df_tofill['Matching_keywords'] = df_tofill[
        'Question_Words'].apply(lambda st: {kwrd for kwrd in st if kwrd in
                                            st_match_words})
    return df_tofill
...
def send(tokenizer=tokenizer):
    if request.method == 'POST':
        ...
        with open(PATH_DATA + 'df_reduced_final.pkl', 'rb') as p:
            df_reduced_final = pickle.load(p)
        ...
        N_ESTIMATORS = 50
        MAX_DEPTH = 100
        columns = list(df_reduced_final.columns)
        add_ind = df_reduced_final.index[-1] + 1
        q_title = request.form['q_title']
        q_body = request.form['q_body']

        # end if no input
        if (q_title == '' or q_body == ''):
            field_empty = "Please, fill all the fields."
            return render_template('form.html', tags=field_empty)
        ...
        fill_df_matching_keywords(df_input)
        df_complete = pd.concat([df_reduced_final, df_input])
        ...
        return render_template('result_tags.html', tags=hi_tags, q_body=q_body,
                               q_title=q_title)
